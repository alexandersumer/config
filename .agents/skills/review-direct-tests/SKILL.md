---
name: review-direct-tests
description: Direct test review without subagents. Use only when the user explicitly asks for direct, inline, single-agent, or no-subagent review of tests or changed behavior.
register_cmd: true
---

# Review Direct Tests

Review and strengthen tests directly in this session. Do not invoke subagents. Preserve the same quality bar as `review-delegated-tests` by identifying changed behavior first, finding the real test seam, and implementing only test improvements that catch named realistic regressions.
