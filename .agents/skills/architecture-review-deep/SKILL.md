...
5. **Validate Reviewer output shape, then cluster and filter.** A Reviewer response is valid only if it uses the requested candidate fields or clearly says no findings for its role. Empty, whitespace-only, truncated, unavailable, timed out, or otherwise unstructured output is invalid. Validate all initial outputs after the initial foreground parallel batch completes. If any Reviewer output is invalid, retry only invalid roles once with a smaller pasted context packet through the same allowed direct reviewer mechanism; when more than one role failed, retry them together in a second foreground parallel batch, and when exactly one role failed, use one direct foreground `functions.invoke_agent` call with `background=false`. Retry only invalid roles; do not rerun valid roles. If any retried role is still invalid, stop with `Review inconclusive`. Never fall back to arbitrary external agent CLIs. Then dedupe by root cause. Drop candidates without file evidence, without a realistic future change path or regression-investigation path, or whose payoff is aesthetic. Validate at most the six strongest directly in this session.
