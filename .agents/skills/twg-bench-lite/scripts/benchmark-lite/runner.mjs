...
const READ_ONLY_TWG_BASH_COMMANDS = [
  ...
  "(?:\\S+/)?twg\\s+jira\\s+workitem\\s+(?:get|query|search|similar|bulk-get)\\b.*",
  "(?:\\S+/)?twg\\s+confluence\\s+(?:content\\s+get|search\\s+(?:query|text)|spaces\\s+(?:get|list|query)|tree\\s+get)\\b.*",
  ...
  "(?:\\S+/)?twg\\s+user\\s+(?:get|search|direct-reports|manager|bulk-lookup)\\b.*",
  "(?:\\S+/)?twg\\s+org-tree\\b.*",
