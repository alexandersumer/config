...
5. Push/commit only when the user explicitly asks to push, publish, commit, or iterate until CI is green. Otherwise finish with local proof and any remote-CI blocker.
...
5. When publish was explicitly authorized, publish only a coherent fix, inspect resulting CI, and repeat this evidence-backed loop for new branch-relevant failures until CI is green or a concrete blocker remains.
...
- Publish/CI: `<not requested | pushed and CI result | blocker>`
