...
1. **Get the effective diff and behavior surface.** Resolve the remote default branch from `origin/HEAD`, falling back to `origin/main` or `origin/master` only if needed. Review the union of committed branch changes, staged changes, unstaged changes, and untracked files: `git diff $(git merge-base <remote-default> HEAD)..HEAD`, `git diff --cached`, `git diff`, and `git ls-files --others --exclude-standard` rendered as new-file diffs, narrowed by `focus_area` or `$ARGUMENTS` if provided. If the committed branch diff is empty but the working tree has staged, unstaged, or untracked changes, review those working-tree changes instead of stopping. If the effective diff is empty, generated-only, formatter-only, version-bump-only, or has no behavior/test relevance, stop with `no test changes justified` and one short reason.
