...
1. Inspect repository context before any write:
   ...
   - push URL for provider and push-target decisions: `git remote get-url --push origin`
...
2. Resolve the PR destination branch with the local-first default branch resolver:
...
3. Determine the PR provider from `git remote get-url --push origin`. Use the push URL for push/provider decisions. If fetch and push URLs point at incompatible providers or repositories such that the review target cannot be identified safely, stop before PR creation and report the exact blocker.
4. Build the effective changes from both sources:
...
5. If there is no branch diff and no working-tree diff, stop.
6. Confirm the combined effective changes form one coherent subject. If the committed branch diff and working-tree diff are unrelated, stop and ask how to split or scope the publish.
7. Choose a valid Conventional Commit subject from the combined effective changes.
8. If currently on `main`, `master`, or the resolved PR destination branch, create a focused branch from the current `HEAD` before staging or committing, named from the subject, for example `fix/short-topic`; do not push local default-branch commits directly to the destination branch.
9. If there are publishable working-tree changes, stage intended changes unless explicitly excluded, then commit them with the chosen subject before pushing. Do not stage unrelated files.
10. If there are no publishable working-tree changes but the source branch already has a branch diff, skip committing and continue.
11. Push the source branch to `origin`, setting upstream if needed.
12. After the source branch has been pushed, always create or report a no-reviewer PR from the source branch to the resolved destination branch using the PR flow below. This applies equally when starting from an existing non-default branch.
...
2. If no PR exists, create one with no reviewers:
   ...
   ```bash
   twg bb prs create --title "<subject>" --source <branch> --dest <destination-branch>
   ```
