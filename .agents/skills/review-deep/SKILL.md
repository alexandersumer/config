...
1. **Get the effective review diff automatically.** Do not require a PR, explicit scope, or committed branch changes. Use the requested base or target PR destination as `<comparison-ref>`; otherwise use a valid `origin/HEAD` or the sole existing `origin/main`/`origin/master` fallback. If no unambiguous base exists, report that gap and review the working-tree portion. Build one effective diff from the union of: committed branch changes with `git diff $(git merge-base <comparison-ref> HEAD)..HEAD`, staged changes with `git diff --cached`, unstaged changes with `git diff`, and untracked files from `git ls-files --others --exclude-standard` rendered as new-file diffs. Always include staged, unstaged, and untracked changes even when the committed branch diff exists. If `focus_area` or `$ARGUMENTS` is provided, use it only to narrow this discovered diff. If the effective review diff is empty, stop with a one-line note. Scale review to the actual risk; draft status, formatting, or version bumps alone do not make a diff unreviewable. Reuse a prior review only when its scope was covered, the relevant code and context are unchanged, and the user has not requested a fresh review.

2. **Read conventions and focused code context yourself.** Find every `CLAUDE.md`, `AGENTS.md`, or `REVIEW.md` whose directory is an ancestor of any changed file. Read enough surrounding unchanged code to understand affected entry points, callers, state/model transitions, public contracts, tests, configs, and failure paths. Put focused excerpts in `{CODE_CONTEXT}`; when the diff is genuinely self-contained, use a one-sentence reason instead of fabricating context.

3. **Invoke four fresh-context Reviewer passes through native managed subagents.** Before invocation, confirm `{DIFF}` contains non-empty pasted diff text or focused excerpts, not a path, filename, or summary, and `{CODE_CONTEXT}` contains focused surrounding context or the explicit self-contained reason. Create one separately prompted context per role. Give each Reviewer only its role prompt and the explicitly constructed evidence packet; do not pass inherited session context, prior Reviewer output, or conclusions from this session. Run roles concurrently when the harness safely supports it. Otherwise run them sequentially or in bounded waves as separate fresh contexts; limited concurrency changes latency, not the review contract. Collect every initial result before validating output or candidates. Do not use external agent CLIs, unmanaged wrappers or processes, detached or scheduled execution, or recursive validation delegation. Each Reviewer pass gets this prompt verbatim, with `{ROLE}`, `{DIFF}`, `{CODE_CONTEXT}`, and `{CONVENTIONS}` filled in:

   > You are reviewing a code change as **{ROLE}**. Diff: {DIFF}. Relevant unchanged caller, contract, and failure-path context: {CODE_CONTEXT}. Conventions: {CONVENTIONS}. One issue = one root cause. Skip nitpicks, style, "consider also". If it is not a real defect or risk, drop it. For changed tests, a low-value or redundant test is a valid issue only when it adds maintenance cost without a named realistic regression signal or duplicates stronger existing coverage. Return exactly one of:
   >
   > CANDIDATES:
