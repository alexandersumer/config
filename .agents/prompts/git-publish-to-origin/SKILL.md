...
Do not create, switch, or rename branches. Work only on the current branch, and push only that branch to `origin`. Do not create or update a pull request as part of this prompt.

Before staging or committing anything, determine the current branch. This prompt intentionally allows publishing the current branch to `origin`, including `main`, `master`, or the remote default branch when the user invokes it.

Inspect the working tree, stage the intended changes, commit using the validated subject as the commit message first line, push the current branch to `origin`, and finish with a brief status summary. Do not treat the chat summary as the commit subject.
